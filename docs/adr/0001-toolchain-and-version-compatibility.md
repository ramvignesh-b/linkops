...
- **Test runner**: **Vitest across the whole workspace**, hand-wired on the five Nest projects and needing no decorator transform plugin — see [ADR-0002](./0002-unified-vitest-runner-and-swc-decorator-metadata.md)
...
| **Test runner** | Vitest, workspace-wide | Jest server-side + Vitest client-side | One `pnpm test`. Costs hand-wiring a `vitest.config.mts` on each of the five Nest projects, and no more than that — [ADR-0002](./0002-unified-vitest-runner-and-swc-decorator-metadata.md) |
...
**7. "Vitest … requires an SWC transform" → no transform plugin is needed.**
Two places in this record — the stack list and the alternatives table — priced the unified runner as costing an SWC transform on the Nest projects. [ADR-0002](./0002-unified-vitest-runner-and-swc-decorator-metadata.md) was corrected on 2026-08-14 when that was finally measured: vite 8 transforms through rolldown/oxc rather than esbuild, oxc honours `emitDecoratorMetadata`, and `unplugin-swc` and `@swc/core` were removed after being proven inert. This record kept the old price for a decision that no longer had it.

The decision itself never changed — Vitest workspace-wide, hand-wired on the five Nest projects — so this is corrected in place, like Correction 5. What changed is the cost, and the cost is what an alternatives table is for: a reader comparing the Jest fallback was being shown a tax that does not exist.

The pattern worth naming: ADR-0002 was rewritten in the commit that disproved it, and the summary of it *here* was not. **A correction has to travel to every record that priced the original claim**, or the contradiction outlives the fix — which is exactly what Correction 5 caught between this record and ADR-0009, in a second place. Found on 2026-08-15 when a spec drafted from a stale summary reintroduced the removed plugin as a prerequisite; `docs/decisions/ai-collaboration.md` entry 22.
