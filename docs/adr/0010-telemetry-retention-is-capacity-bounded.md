...
- **A third site, added with the Link detail route.** The Console holds its own bound, `HISTORY_CAP` in `libs/console/feature-link-detail/src/lib/link-history.ts`, because a client that kept appending live Samples past what the Server retains would grow with how long a screen stayed open — the Leak this decision closes on the Server, reopened on the Console. It is the same number for the same reason, and lifting it into `shared/domain` to share it is the option rejected above. Changing the retention bound now means grepping for all three.
